...
if ( ! class_exists( 'Email_Webhook_Filter', false ) ) {
    ...
    class Email_Webhook_Filter {
        ...
        public function register_settings() {
            ...
            add_settings_field(
                'always_success',
                __( 'Always Succeed wp_mail()', 'email-webhook-filter' ),
                array( $this, 'always_success_field' ),
                'email-webhook-filter',
                'email_webhook_filter_main_section'
            );
        }
        ...
        public function sanitize_settings( $input ) {
            ...
            $sanitized['always_success'] = isset( $input['always_success'] ) ? 1 : 0;
		...
		/**
		 * Get EWF_URL from environment (sanitized).
		 *
		 * @return string
		 */
		private function get_env_webhook_url() {
			$env = getenv( 'EWF_URL' );
			if ( false === $env || '' === $env ) {
				return '';
			}
		 ...
		 * @return string
		 */
		private function resolve_webhook_url( $settings ) {
			$url = isset( $settings['webhook_url'] ) ? trim( $settings['webhook_url'] ) : '';
			if ( '' !== $url ) {
				return $url;
			}
        ...
        private function send_webhook( $payload, $settings ) {
			...
			$webhook_url = $this->resolve_webhook_url( $settings );
        ...
        public function maybe_short_circuit_mail( $short_circuit, $atts ) {
            $settings = $this->get_settings();
            if ( empty( $settings['always_success'] ) ) {
        ...
        public function webhook_url_field() {
			...
			$env_placeholder = $this->get_env_webhook_url();
            echo '<input type="text" class="regular-text" name="email_webhook_filter_settings[webhook_url]" value="' . esc_attr( $webhook_url ) . '" ' . ( $env_placeholder ? 'placeholder="' . esc_attr( $env_placeholder ) . '" ' : '' ) . '>';
        }
         ...
         */
        public function always_success_field() {
            $options = get_option( 'email_webhook_filter_settings' );
            $enabled = isset( $options['always_success'] ) ? (int) $options['always_success'] : 1;
            echo '<label><input type="checkbox" name="email_webhook_filter_settings[always_success]" value="1" ' . checked( 1, $enabled, false ) . '> ' . esc_html__( 'When enabled, "wp_mail()" returns "true" even if delivery fails. Use with caution.', 'email-webhook-filter' ) . '</label>';
        }
    }
